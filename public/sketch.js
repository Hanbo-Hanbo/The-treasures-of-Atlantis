...
// --- Debug & Audio ---
let rawLat = "N/A", rawLon = "N/A", gpsAccuracy = "N/A", rawHeading = "N/A";
...
function setup() {
    ...
    // 关键：当服务器广播 update-bases 时，客户端必须强制覆盖本地列表
...
function mousePressed() {
    if (gameState === 0) {
        ...
        // 预先在本地画一个（防止网络延迟），等服务器 update-bases 回来后会自动修正
        bases.push(myBase);
...
function handleOrientation(e) {
    ...
    } else if (e.absolute || typeof e.absolute === 'undefined') {
        heading = 360 - e.alpha;
    }
...
function drawPlayerMarker(x, y, col, isMe) {
    ...
    if (isMe) {
        ...
        if (nearest) {
            ...
            push(); rotate(angleToTreasure);
            stroke(255, 0, 0); strokeWeight(3); line(0, 0, 0, -28); 
            fill(255, 0, 0); noStroke(); triangle(-6, -20, 6, -20, 0, -32);
            pop();
        }
        // White Field of View (Phone direction)
        push(); rotate(radians(heading));
        noStroke(); fill(255, 30); arc(0, 0, 80, 80, radians(-30), radians(30), PIE);
        pop();
...
function getNearestTreasure() {
    ...
    treasures.forEach(t => {
        if (!t.found) {
            let d = dist(myPos.x, myPos.y, t.x, t.y);
            if (d < record) { record = d; closest = t; }
        }
    });
    return closest;
}
...
function drawDiagnostic() { push(); textAlign(LEFT, BOTTOM); textSize(9); fill(255, 120); text(`DIR: ${rawHeading} | GPS: ${rawLat}, ${rawLon} | ACC: ${gpsAccuracy}`, 15, height - 15); pop(); }
