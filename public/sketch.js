...
// --- Config ---
const GAME_RANGE_METERS = 7.07; // 50m2 square root
const LERP_FACTOR = 0.08;       // Smoothing speed (0.0 to 1.0)
...
function draw() {
    ...
    if (gameState === 0) {
        ...
        drawGameOverTable(); // Final Ranking Table
    }
}
...
// --- Ranking Table UI ---
function drawGameOverTable() {
    fill(0, 230); rect(0, 0, width, height);
    push();
    fill(0); stroke(255); strokeWeight(2);
    rect(width/2 - 120, height/2 - 150, 240, 300, 10);
    
    noStroke(); fill(255); textAlign(CENTER); textSize(24);
    text("GAME OVER", width/2, height/2 - 110);
    textSize(14);
    text("FINAL RANKING", width/2, height/2 - 80);
    
    textAlign(LEFT);
    finalRanking.forEach((p, i) => {
        fill(p.color.r, p.color.g, p.color.b); circle(width/2 - 80, height/2 - 40 + i * 35, 12);
        fill(255); text(`Rank ${i+1}: ${p.score} Stars`, width/2 - 50, height/2 - 35 + i * 35);
    });
    
    textAlign(CENTER); textSize(10);
    text("NEW ROUND STARTING SOON...", width/2, height/2 + 130);
    pop();
}

function drawPlayerMarker(x, y, col, isMe) {
    push(); translate(x, y); fill(col.r, col.g, col.b); stroke(255); strokeWeight(isMe ? 3 : 1);
    circle(0, 0, 18);
    if (isMe) {
...
function drawDistanceBar(d) {
    ...
    noStroke(); rect(0, height, barW, -map(d, 0, 0.8, height, 0));
}
